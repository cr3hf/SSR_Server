...
Download_SSR(){
	...
	mkdir ${ssr_folder} -p
	chmod 777 ${ssr_folder}
	wget --no-check-certificate https://raw.githubusercontent.com/cr3hf/SSR_Server/master/ssr.tar -O ${ssr_folder}/ssr.tar

	tar -xvf ${ssr_folder}/ssr.tar -C ${ssr_folder}
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/cr3hf/SSR_Server/master/doubi-master/other/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/cr3hf/SSR_Server/master/doubi-master/other/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		if [[ ${bit} = "x86_64" ]]; then
			wget --no-check-certificate https://raw.githubusercontent.com/cr3hf/SSR_Server/master/jq-linux64 -O ${jq_file}
		else
			wget --no-check-certificate https://raw.githubusercontent.com/cr3hf/SSR_Server/master/jq-linux32 -O ${jq_file}
		fi
