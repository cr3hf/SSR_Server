...
Download_SSR(){
	...
	wget --no-check-certificate ftp://hyn.hotweb.win:65521/AiCard_01/Everyone/SSR/ssr.tar -O ${ssr_folder}/ssr.tar
...
JQ_install(){
	if [[ ! -e ${jq_file} ]]; then
		if [[ ${bit} = "x86_64" ]]; then
			wget --no-check-certificate ftp://hyn.hotweb.win:65521/AiCard_01/Everyone/SSR/jq-linux64 -O ${jq_file}
		else
			wget --no-check-certificate ftp://hyn.hotweb.win:65521/AiCard_01/Everyone/SSR/jq-linux32 -O ${jq_file}
		fi
